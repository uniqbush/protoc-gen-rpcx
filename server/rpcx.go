...
func (p *rpcx) Generate(file *generator.FileDescriptor) {
	...
	for i, service := range file.FileDescriptorProto.Service {
		...
		err := ioutil.WriteFile(fmt.Sprintf("cmd/%sserver_autogen.go", serviceName), p.gen.Bytes(), 0644)
		if err != nil {
			panic(err)
		}
	}
...
func (p *rpcx) generateService(file *generator.FileDescriptor, service *pb.ServiceDescriptorProto, index int) {
	...
	for _, method := range service.Method {
		bak := &rpcx{gen: p.gen}
		bak.generateServerCode(service, method)
		...
		err := ioutil.WriteFile(fmt.Sprintf("internel/%s.go", fileName), p.gen.Bytes(), 0644)
		if err != nil {
			panic(err)
		}
	}
